...
15. **Mascot placement:** When the project CONTENT-GENERATION-GUIDE.md defines a mascot, follow its rendered mascot placement rules exactly — pose selection, per-chapter counts, hard limits, and the one-time Chapter 1 self-introduction. Skip entirely if no mascot is defined. See Step 2.4 principle 4.
